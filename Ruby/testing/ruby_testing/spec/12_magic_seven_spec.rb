...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the a MagicSeven instance is stubbed with 2 and previous step is 6' do
      subject(:stubbed_number_game) { described_class.new(2) }
      it 'returns 4' do
        previous_step = 6
        result = stubbed_number_game.subtract_random_number(previous_step)
  ...
  describe '#play' do
    ...
    subject(:stubbed_number_game) { described_class.new(2) }
      it 'will return 7' do
        result = stubbed_number_game.play
        expect(result).to equal(7)
      end
    end

    context 'when the random number is 310' do
    subject(:stubbed_number_game) { described_class.new(310) }
      it 'will return 7' do
        result = stubbed_number_game.play
        expect(result).to equal(7)
      end
    ...
    context 'when the random number is 123456789' do
    subject(:stubbed_number_game) { described_class.new(123456789) }
      it 'will return 7' do
        result = stubbed_number_game.play
        expect(result).to equal(7)
      end
